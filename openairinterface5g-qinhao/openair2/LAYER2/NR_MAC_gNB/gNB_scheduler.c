...
*/
int get_last_ul_tti_req_ind(gNB_MAC_INST * gNB, frame_t frame, sub_frame_t slot)
{
...
void clear_nr_nfapi_information(gNB_MAC_INST * gNB,
  ...
  int last_ind = get_last_ul_tti_req_ind(gNB, frameP, slotP);
  nfapi_nr_ul_tti_request_t    *future_ul_tti_req =
      &gNB->UL_tti_req_ahead[CC_idP][last_ind];
  ...
  future_ul_tti_req->SFN = (slotP == 0 ? frameP + 1: frameP + 2) % 1024;
  LOG_D(MAC,"Future_ul_tti SFN = %d for slot %d \n", future_ul_tti_req->SFN, last_ind);
  ...
  */
  ...
  gNB->UL_tti_req[CC_idP] = &gNB->UL_tti_req_ahead[CC_idP][future_ind];
...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP,
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    const int last_slot = get_last_ul_tti_req_ind(gNB, frame, slot);
