...
int get_future_ul_tti_req_ind(frame_t frame, sub_frame_t slot);
...
void schedule_nr_prach(module_id_t module_idP, frame_t frameP, sub_frame_t slotP)
{
  ...
  int future_index = get_future_ul_tti_req_ind(frameP, slotP);  
  nfapi_nr_ul_tti_request_t *UL_tti_req = &RC.nrmac[module_idP]->UL_tti_req_ahead[0][future_index];
  nfapi_nr_config_request_scf_t *cfg = &RC.nrmac[module_idP]->config[0];
  ...
  if (is_nr_UL_slot(scc->tdd_UL_DL_ConfigurationCommon, slotP, cc->frame_type)) {
    ...
    if ( get_nr_prach_info_from_index(config_index,
      ...
      int future_ind = get_future_ul_tti_req_ind(frameP, slotP);
      uint16_t *vrb_map_UL = &cc->vrb_map_UL[future_ind * MAX_BWP_SIZE];
...
// number of mixed slot or of last dl slot if there is no mixed slot
  ...
  uint8_t frame_ahead = num_delay / nr_slots_per_frame[mu] + 1;
  // slot and frame limit to transmit msg2 according to response window
  uint8_t slot_limit = (rach_slot + slot_window)%nr_slots_per_frame[mu];
  uint8_t frame_limit = (rach_frame + frame_ahead)%1024 ;
  ...
  *msg2_frame = ((*msg2_slot>(rach_slot))? rach_frame : (rach_frame + frame_ahead))%1024; 
...
void nr_generate_Msg3_retransmission(module_id_t module_idP, int CC_id, frame_t frame, sub_frame_t slot, NR_RA_t *ra) {
  ...
  const int sched_frame = (frame + (slot + K2 + num_delay) /  nr_slots_per_frame[mu])%1024;
  const int sched_slot = (slot + K2 + num_delay) % nr_slots_per_frame[mu];
  ...
  if (is_xlsch_in_slot(RC.nrmac[module_idP]->ulsch_slot_bitmap[sched_slot / 64], sched_slot)) {
    ...
    int future_ind = get_future_ul_tti_req_ind(sched_frame, sched_slot);
    uint16_t *vrb_map_UL = &RC.nrmac[module_idP]->common_channels[CC_id].vrb_map_UL[future_ind * MAX_BWP_SIZE];
    ...
    int future_index = get_future_ul_tti_req_ind(sched_frame, sched_slot);

    nfapi_nr_ul_tti_request_t *future_ul_tti_req = &RC.nrmac[module_idP]->UL_tti_req_ahead[CC_id][future_index];
...
void nr_get_Msg3alloc(module_id_t module_id,
  ...
  for (int i=0; i<pusch_TimeDomainAllocationList->list.count; i++) {
    ...
    if (NrOfSymbols == scc->tdd_UL_DL_ConfigurationCommon->pattern1.nrofUplinkSymbols) {
      ...
      temp_slot = current_slot + k2 + DELTA[mu] + num_delay; // msg3 slot according to 8.3 in 38.213
  ...
  int future_ind = get_future_ul_tti_req_ind(ra->Msg3_frame, ra->Msg3_slot);
  uint16_t *vrb_map_UL =
      &RC.nrmac[module_id]->common_channels[CC_id].vrb_map_UL[future_ind * MAX_BWP_SIZE];
...
void nr_add_msg3(module_id_t module_idP, int CC_id, frame_t frameP, sub_frame_t slotP, NR_RA_t *ra, uint8_t *RAR_pdu)
{
  ...
  int future_ind = get_future_ul_tti_req_ind(ra->Msg3_frame, ra->Msg3_slot);
  uint16_t *vrb_map_UL =
      &RC.nrmac[module_idP]->common_channels[CC_id].vrb_map_UL[future_ind * MAX_BWP_SIZE];
  ...
  int future_index = get_future_ul_tti_req_ind(ra->Msg3_frame, ra->Msg3_slot);
  nfapi_nr_ul_tti_request_t *future_ul_tti_req = &RC.nrmac[module_idP]->UL_tti_req_ahead[CC_id][future_index];
