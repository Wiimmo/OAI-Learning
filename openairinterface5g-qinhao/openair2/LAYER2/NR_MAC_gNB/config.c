...
int rrc_mac_config_req_gNB(module_id_t Mod_idP,
  ...
  if (scc != NULL ) {
    ...
    const int n = slots_per_frame[*scc->ssbSubcarrierSpacing] + num_delay;
    ...
    for (int i = 0; i < n; ++i) {
       ...
       * happens now, too) */
      req->SFN = i < sl_ahead;
      req->Slot = i % slots_per_frame[*scc->ssbSubcarrierSpacing];
    }
    ...
    for (int slot = 0; slot < n; ++slot) {
      ...
      if (slot % nr_slots_period != 0)
        RC.nrmac[Mod_idP]->dlsch_slot_bitmap[slot / 64] |= (uint64_t)((slot % nr_slots_period) < nr_dlmix_slots-1) << (slot % 64);
