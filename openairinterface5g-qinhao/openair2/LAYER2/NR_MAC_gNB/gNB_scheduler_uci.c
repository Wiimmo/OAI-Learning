                         ...
                         int UE_id)
{
  ...
  int future_index = get_future_ul_tti_req_ind(pucch->frame, pucch->ul_slot);
  nfapi_nr_ul_tti_request_t *future_ul_tti_req =
      &RC.nrmac[mod_id]->UL_tti_req_ahead[0][future_index];
...
void nr_csi_meas_reporting(int Mod_idP,
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    for (int csi_report_id = 0; csi_report_id < csi_measconfig->csi_ReportConfigToAddModList->list.count; csi_report_id++){
      ...
      int future_ind = get_future_ul_tti_req_ind(frame, sched_slot);
      uint16_t *vrb_map_UL = &RC.nrmac[Mod_idP]->common_channels[0].vrb_map_UL[future_ind * MAX_BWP_SIZE];
...
static NR_UE_harq_t *find_harq(module_id_t mod_id, frame_t frame, sub_frame_t slot, int UE_id)
{
  ...
  while ((harq->feedback_frame)%1024 != frame
...
int nr_acknack_scheduling(int mod_id,
  ...
  if (pucch_Config) {
    ...
    int future_ind = get_future_ul_tti_req_ind(pucch->frame, pucch->ul_slot);
    if (resource->format.choice.format0->initialCyclicShift == 0) {
      uint16_t *vrb_map_UL = &RC.nrmac[mod_id]->common_channels[CC_id].vrb_map_UL[future_ind * MAX_BWP_SIZE];
...
void nr_sr_reporting(int Mod_idP, frame_t SFN, sub_frame_t slot)
{
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    for (int SR_resource_id =0; SR_resource_id < pucch_Config->schedulingRequestResourceToAddModList->list.count;SR_resource_id++) {
      ...
      int future_index = get_future_ul_tti_req_ind(SFN, slot); 
      nfapi_nr_ul_tti_request_t *ul_tti_req = &nrmac->UL_tti_req_ahead[0][future_index];
