      ...
      LOG_D(NR_MAC, "[UE %d] PUSCH TPC %d and TA %d\n",UE_id,UE_scheduling_control->tpc0,UE_scheduling_control->ta_update);
        if(ra->cfra) {
          ...
          if (nr_process_mac_pdu(gnb_mod_idP, UE_id, CC_idP, frameP, slotP, sduP, sdu_lenP) == 0) {
            ra->state = Msg4;
            ra->Msg4_frame = (frameP + (sl_ahead + num_delay)/nr_slots_per_frame[mu] + 1) % 1024;
            ra->Msg4_slot = 1;
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    int future_index = get_future_ul_tti_req_ind(sched_pusch->frame, sched_pusch->slot);
    nfapi_nr_ul_tti_request_t *future_ul_tti_req = &RC.nrmac[module_id]->UL_tti_req_ahead[0][future_index];
