...
static int get_nr_prach_info_from_ssb_index(uint8_t ssb_idx,
  ...
  *prach_occasion_info_pp = NULL;
  ...
  int prach_slot = (slot + 2*slot_delay) % 20;
  int prach_frame = (frame + (slot + 2*slot_delay) / 20) % 1024;
  ...
  for (uint8_t n_mapped_ro=0; n_mapped_ro<ssb_info_p->nb_mapped_ro; n_mapped_ro++) {
    ...
    if ((prach_slot == ssb_info_p->mapped_ro[n_mapped_ro]->slot) &&
        (ssb_info_p->mapped_ro[n_mapped_ro]->frame == (prach_frame % prach_assoc_pattern.nb_of_frame))) {
      ...
      prach_occasion_slot_p = &prach_assoc_pattern.prach_conf_period_list[prach_config_period_nb].prach_occasion_slot_map[frame_nb_in_prach_config_period][prach_slot];
    }
  }
