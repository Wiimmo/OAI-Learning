...
typedef struct nfapi_pnf_p7_config
{
	...
	int (*dl_tti_proc_fn)(nfapi_pnf_p7_config_t* config, nfapi_nr_dl_tti_request_t* dtr, nfapi_nr_tx_data_request_t* tdr, nfapi_nr_ul_dci_request_t* udr);

	/*! A callback for the DL_TX_CONFLICT if there is a dl_tti_req and a ul_dci_req at the same slot
	 * \param msgtx A struct contain tx message
	 * \param config A poiner to the PNF P7 config
	 * \param req1 A pointer to the ul request message structure
	 * \param req2 A pointer to the ul request message structure
	 * \return not currently used	
	 */
	int (*dl_tx_conflict)(processingData_L1tx_t *msgtx, nfapi_pnf_p7_config_t* config, nfapi_nr_ul_dci_request_t* ul_dci_req, nfapi_nr_dl_tti_request_t* dl_tti_req);
