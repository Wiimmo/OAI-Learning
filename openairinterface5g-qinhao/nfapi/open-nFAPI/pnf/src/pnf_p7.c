...
{
	printf("deallocate_nfapi_tx_data_request is called");
/*
	if(pnf_p7->_public.codec_config.deallocate)
	{
...
{	
	...
	//		NFAPI_TRACE(NFAPI_TRACE_INFO, "Applying shift %d to sfn/slot (%d -> %d)\n", pnf_p7->sfn_slot_shift, NFAPI_SFNSF2DEC(sfn_slot), shifted_sfn_slot);
...
#else
		nfapi_nr_tx_data_request_t* tdr = tx_slot_buffer->tx_data_req;
		if(tdr != 0 && tdr->SFN == sfn_tx && tdr->Slot == slot_tx) {
		}
		else {
			if(pnf_p7->_public.dummy_slot.tx_data_req)
			{
				pnf_p7->_public.dummy_slot.tx_data_req->SFN = sfn_tx;
				pnf_p7->_public.dummy_slot.tx_data_req->Slot = slot_tx; 
				tdr = pnf_p7->_public.dummy_slot.tx_data_req;
			}
		}
				
		nfapi_nr_dl_tti_request_t* dtr = tx_slot_buffer->dl_tti_req;
		if(dtr != 0 && dtr->SFN == sfn_tx && dtr->Slot == slot_tx) {
		}
		else {// send dummy
			if(pnf_p7->_public.dummy_slot.dl_tti_req) {   
				pnf_p7->_public.dummy_slot.dl_tti_req->SFN = sfn_tx;
				pnf_p7->_public.dummy_slot.dl_tti_req->Slot = slot_tx;
				dtr = pnf_p7->_public.dummy_slot.dl_tti_req;
			}
		}

		nfapi_nr_ul_dci_request_t* udr = tx_slot_buffer->ul_dci_req;
		if(udr!= 0 && udr->SFN == sfn_tx && udr->Slot == slot_tx) {
		}
		else { //send dummy
			if(pnf_p7->_public.dummy_slot.ul_dci_req) {
				pnf_p7->_public.dummy_slot.ul_dci_req->SFN = sfn_tx;
				pnf_p7->_public.dummy_slot.ul_dci_req->Slot = slot_tx;
				udr = pnf_p7->_public.dummy_slot.ul_dci_req;
			}
		}

		if(pnf_p7->_public.dl_tti_proc_fn) {	
			//NFAPI_TRACE(NFAPI_TRACE_INFO, "Calling tx_data_req_fn in SFN/slot %d.%d \n",sfn,slot);
			LOG_D(PHY, "Process tx_data SFN/slot %d.%d buffer index: %d \n",sfn_tx,slot_tx,buffer_index_tx);	
			(pnf_p7->_public.dl_tti_proc_fn)(&(pnf_p7->_public), dtr, tdr, udr);
		}
		...
		{
			printf("tx_slot_buffer->tx_data_req is not 0\n");
		...
		if(tx_slot_buffer->ul_tti_req != 0 && tx_slot_buffer->ul_tti_req->SFN == sfn && tx_slot_buffer->ul_tti_req->Slot == slot_tx)
		{
