...
int pnf_connect(pnf_t* pnf)
{
	...
	while(p != NULL && connected == 0)
	{
		...
		client_addr.sin_addr.s_addr =inet_addr(pnf->_public.pnf_ipv4);
		client_addr.sin_family = AF_INET;
		client_addr.sin_port = htons(pnf->_public.pnf_p5_port);//固定端口号
		memset(&(client_addr.sin_zero), 0, sizeof(client_addr.sin_zero));
		/* bind */
		if (bind(pnf->p5_sock, (struct sockaddr *)&client_addr, sizeof(struct sockaddr_in)) < 0)
		{
			NFAPI_TRACE(NFAPI_TRACE_ERROR, "bind failed..: %d\n", errno);
			return -1;
		}
		NFAPI_TRACE(NFAPI_TRACE_INFO, "P5 socket created, bind to: %s:%d\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
