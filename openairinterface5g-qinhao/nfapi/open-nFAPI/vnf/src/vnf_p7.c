...
int vnf_send_p7_msg(vnf_p7_t* vnf_p7, nfapi_vnf_p7_connection_info_t* p7_info, uint8_t* msg, const uint32_t len)
{
	...
	{
		NFAPI_TRACE(NFAPI_TRACE_WARN, "%s() sendto_result %d %d\n", __FUNCTION__, sendto_result, errno);
	}
	// char *ll = inet_ntoa(p7_info->remote_addr.sin_addr);
	// char nnn[20];
	// memset(nnn, 0, 20);
	// strncpy(nnn, ll, 16);
	// printf("XXXXXXXXXXXXXX %s:%d\n", nnn, ntohs(p7_info->remote_addr.sin_port));
	return 0;
}
