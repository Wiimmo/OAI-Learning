...
int nfapi_nr_vnf_start(nfapi_vnf_config_t* config)
{
	...
	*/
	...
	else if(config->vnf_ipv4)
	{
		...
		addr.sin_addr.s_addr = inet_addr(config->vnf_p5_ipv4);
	...
	NFAPI_TRACE(NFAPI_TRACE_INFO, "vnf p5 bind to %s:%d succeeded..%d.\n",inet_ntoa(addr.sin_addr), ntohs(addr.sin_port), p5ListenSock);
