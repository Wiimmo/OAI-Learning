...
int param_request(nfapi_pnf_config_t *config, nfapi_pnf_phy_config_t *phy, nfapi_param_request_t *req) {
  ...
  nfapi_resp.nfapi_config.p7_pnf_port.value = pnf->phys[0].udp.rx_port; // DJP - hard code alert!!!! FIXME TODO
  nfapi_resp.num_tlv++;
...
*/
  ...
  nfapi_resp.nfapi_config.p7_pnf_port.value = pnf->phys[0].udp.rx_port; //pnf->phys[0].local_port; DJP - hard code alert!!!! FIXME TODO
  nfapi_resp.num_tlv++;
  pnf_p7_sockaddr.sin_addr.s_addr = inet_addr(pnf->phys[0].local_addr);
...
notifiedFIFO_elt_t *l1tx_message_extract(PHY_VARS_gNB *gNB, int frame, int slot) {
  ...
  if (freeRes) {
    msgTx = (processingData_L1tx_t *)NotifiedFifoData(freeRes);
...
int pnf_phy_dl_tti_req(gNB_L1_rxtx_proc_t *proc, nfapi_pnf_p7_config_t *pnf_p7, nfapi_nr_dl_tti_request_t *req) {
  ...
  int slot_type = nr_slot_select(cfg,sfn,slot);
...
int pnf_phy_dl_tti_proc(nfapi_pnf_p7_config_t* config, nfapi_nr_dl_tti_request_t* dtr, nfapi_nr_tx_data_request_t* tdr, nfapi_nr_ul_dci_request_t* udr)
{
  if (RC.ru == 0) {
    return -1;
  }
  ...
  if (slot_type == NR_DOWNLINK_SLOT || slot_type == NR_MIXED_SLOT) {
    ...
    processingData_L1tx_t *msgTx = (processingData_L1tx_t *)NotifiedFifoData(res);
    if(dtr) {
      nfapi_nr_dl_tti_request_pdu_t *dl_tti_pdu_list = dtr->dl_tti_request_body.dl_tti_pdu_list;
      for (int i=0; i<dtr->dl_tti_request_body.nPDUs; i++) {
        // NFAPI_TRACE(NFAPI_TRACE_INFO, "%s() sfn/sf:%d PDU[%d] size:%d pdcch_vars->num_dci:%d\n", __FUNCTION__, NFAPI_SFNSF2DEC(dtr->sfn_sf), i, dl_config_pdu_list[i].pdu_size,pdcch_vars->num_dci);
        ...
        if (dl_tti_pdu_list[i].PDUType == NFAPI_NR_DL_TTI_PDCCH_PDU_TYPE) {
          ...
          nfapi_nr_dl_tti_pdsch_pdu *pdsch_pdu = &dl_tti_pdu_list[i].pdsch_pdu;
          nfapi_nr_dl_tti_pdsch_pdu_rel15_t *rel15_pdu = &pdsch_pdu->pdsch_pdu_rel15;     
          nfapi_nr_pdu_t *tx_data = NULL;
          if(tdr) 
            tx_data = &tdr->pdu_list[rel15_pdu->pduIndex];
    ...
    if (udr && (udr->numPdus > 0)) {
      if (udr->ul_dci_pdu_list[udr->numPdus-1].PDUType == 0) { // copy only the last PDU (PHY can have only one UL PDCCH pdu)
        msgTx->ul_pdcch_pdu = udr->ul_dci_pdu_list[udr->numPdus-1]; // copy the last pdu
      } 
      else {
        LOG_E(PHY,"[PNF] UL_DCI_REQ sfn_slot:%d PDU[%d] - unknown pdu type:%d\n", NFAPI_SFNSLOT2DEC(udr->SFN, udr->Slot), udr->numPdus-1, udr->ul_dci_pdu_list[udr->numPdus-1].PDUType);
      }
    }
  ...
  if(dtr->vendor_extension)
    free(dtr->vendor_extension);

  return 0;
}


int pnf_phy_dl_tx_conflict(processingData_L1tx_t *msgTx, nfapi_pnf_p7_config_t *pnf_p7, nfapi_nr_ul_dci_request_t* ul_dci_req, nfapi_nr_dl_tti_request_t* dl_tti_req){

  // process ul_dci_req
  if (ul_dci_req->numPdus > 0) {
    if (ul_dci_req->ul_dci_pdu_list[ul_dci_req->numPdus-1].PDUType == 0) { // copy only the last PDU (PHY can have only one UL PDCCH pdu)
      msgTx->ul_pdcch_pdu = ul_dci_req->ul_dci_pdu_list[ul_dci_req->numPdus-1]; // copy the last pdu
    } 
    else {
      LOG_E(PHY,"[PNF] UL_DCI_REQ sfn_slot:%d PDU[%d] - unknown pdu type:%d\n", NFAPI_SFNSLOT2DEC(ul_dci_req->SFN, ul_dci_req->Slot), ul_dci_req->numPdus-1, ul_dci_req->ul_dci_pdu_list[ul_dci_req->numPdus-1].PDUType);
    }
  }

  // process dl_tti_req
  struct PHY_VARS_gNB_s *gNB = RC.gNB[0];
  int sfn = dl_tti_req->SFN;
  int slot = dl_tti_req->Slot;
  nfapi_nr_config_request_scf_t *cfg = &gNB->gNB_config;
  int slot_type = nr_slot_select(cfg,sfn,slot);

  nfapi_nr_dl_tti_request_pdu_t *dl_tti_pdu_list = dl_tti_req->dl_tti_request_body.dl_tti_pdu_list;
...
int start_request(nfapi_pnf_config_t *config, nfapi_pnf_phy_config_t *phy, nfapi_start_request_t *req) {
  ...
  p7_config->local_p7_port = phy_info->udp.rx_port;//phy_info->local_port;//50010; // DJP - good grief cannot seem to get the right answer phy_info->local_port;
...
static nfapi_nr_dl_tti_request_t dummy_dl_tti_req;
int nr_start_request(nfapi_pnf_config_t *config, nfapi_pnf_phy_config_t *phy,  nfapi_nr_start_request_scf_t *req) {
  ...
  p7_config->local_p7_port = phy_info->udp.rx_port; // DJP - good grief cannot seem to get the right answer phy_info->local_port;
  //DJP p7_config->local_p7_addr = (char*)phy_info->local_addr.c_str();
  p7_config->local_p7_addr = phy_info->local_addr;
  printf("[PNF] P7 remote:%s:%d local:%s:%d\n", p7_config->remote_p7_addr, p7_config->remote_p7_port, p7_config->local_p7_addr, p7_config->local_p7_port);
  p7_config->user_data = phy_info;
  ...
  p7_config->dl_tti_proc_fn = &pnf_phy_dl_tti_proc;
  p7_config->dl_tx_conflict = &pnf_phy_dl_tx_conflict;
  ...
  dummy_slot.ul_tti_req = 0;//&dummy_tx_req;
...
void configure_nr_nfapi_pnf(char *vnf_ip_addr, int vnf_p5_port, char *pnf_ip_addr, int pnf_p7_port, int vnf_p7_port) {
  ...
  config->pnf_ipv4 = pnf_ip_addr;
  config->pnf_p5_port = 50000;
...
void handle_nr_slot_ind(uint16_t sfn, uint16_t slot) {
    ...
    // printf("[PNF] Sent slot indication %d.%d\n", sfn, slot);
